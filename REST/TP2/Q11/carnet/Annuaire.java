...
public class Annuaire{
	...
	@GET
	@Produces("text/html")
	public Response getContact(@PathParam("name") String name) throws ContactNotFoundException {
		for(Contact contact : this.carnet.getContacts()){
			if(contact.getNom().equals(name))
				return Response.status(Response.Status.OK).entity(contact.getNumero()).build() ;
		}
	...
	@GET
	@Produces("text/html")
	public Response deleteByNmae(@PathParam("nom") String nom){
		for(Contact contact : this.carnet.getContacts()){
			if(contact.getNom().equals(nom)){
				this.carnet.getContacts().remove(contact);
				
               
				return Response.status(Response.Status.OK).cookie(new NewCookie("contact", "Nom: "+contact.getNom()+" : Numéro : "+contact.getNumero())).build() ;
			}
		}
		return (new ContactNotFoundException()).getResponse();	
	...
	@GET
	@Produces("text/html")
	public Response displayForLastTime(@CookieParam("contact")  String value){
